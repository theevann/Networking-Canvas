...
public class ImageDessin {
	...
	static int freq = 10;// freq = la periode e laquelle on cree une nouvelle image 
	...
	BufferedImage img;
	int id = -1; // id = numero  du dernier motif dessine dans la liste des motifs
	...
	//Cette classe permet de creer l'image actuelle et les images temporaires e partir d'un certain rang de la liste des motifs
	 public static BufferedImage creerImageDepuis(int idMotif){ // idMotif = num du motif dans la liste
		 ...
		 imgAUtilise = (idMotif+1) - ((idMotif+1)%freq); //Ici, on choisit le rang du motif e partir duquel on va redessiner pour utiliser une image deje creee.
		 
		 //System.out.println("Image utilisee : " + imgAUtilise/freq + " (" + imgAUtilise + ")");
		 ...
		 synchronized (listeMotif) {
		     ...
		     while(iter.hasNext()){
		    	 ...
		    	 if((i+1)%freq == 0){ // Si on est multiple de la periode, on enregistre l'image
	 ...
	 public static void undo(int [] idASupprimer){
		synchronized (listeMotif) {
		 ...
		 creerImageDepuis(idASupprimer[0] - 1); // On demande e recreer l'image actuelle (et par la meme occasion les images intermediaires) e partir du rang du motif precedant le 1er motif supprime
		 nb++;
	 }
