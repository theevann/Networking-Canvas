...
public class Canevas extends JPanel{
	...
	public void paintComponent(Graphics g) {
		  ...
		  /**
		   * Gestion du clignotement des éléments séléctionnés
		   */
		  
		  //Si des colonnes sont selectionnés et que la variable clignotement (mis à jour par le thread) est à true, on va dessiner des pointillés
		  if(OngletHistorique.ongletActuel.getTableau().getSelectedRowCount() > 0 && clignotement){
			  ...
			  // On récupère le tableau Jtable
			  JTable tableau = OngletHistorique.ongletActuel.getTableau();
			  // On prend dans le Jtable le numero des motifs à redessiner en pointillés -> on enregistre ces numéros dans un tableu d'int
			  ...
			  for(int i = 0 ; i < tableau.getSelectedRowCount() ; i++)
				...
				//On le trie, ici pas très utile
				Arrays.sort(tableauADessine);
				//Pour chaque motif dont le rang dans la liste listeMotif est dans le tableau d'int (cad pour chaque motif sélectionné), on demande à le redessiner en "mode selectionné", cad en pratique en pointillés
				for(int i : tableauADessine){
					...
					}catch(IndexOutOfBoundsException e){} // Parfois problème lors du parcours de la boucle qd quelquechose est supprimé en même temps... Pour ne pas avoir à synchroniser (inutile ici), on gère juste l'exception
				}
		  }
...
class EcouteurSourisCanevas extends MouseAdapter{
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(MainClient.fenetre.getMotifSelectionne() == TypeMotif.TEXTE){
			...
			String texte = jop.showInputDialog(null, "Texte à insérer : ", "Ecrire du texte", JOptionPane.QUESTION_MESSAGE);
			if(texte != null && texte != ""){
	...
	public void mouseExited(MouseEvent e) {
		//Si on quitte la zone de dessin, on considère le dessin comme annulé. En plus, il est plus compliqué de recupérer l'information mouseReleased en dehors du Canevas
	...
	public void mouseReleased(MouseEvent e) {
		...
		if(canevas.clicEnCours && MainClient.client.isConnecte() && canevas.getMotifEnCours().getType() != null && MainClient.fenetre.getMotifSelectionne() != TypeMotif.SELECTION && MainClient.fenetre.getMotifSelectionne() != TypeMotif.DEPLACER){
			//Si c'est un feutre, on met plein que quand on envoie, sinon c'est incompréhensible quand on dessine
			if((canevas.getMotifEnCours() instanceof Feutre))
