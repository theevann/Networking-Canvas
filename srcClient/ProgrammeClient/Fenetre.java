...
public class Fenetre extends JFrame {
	...
	JButton bConnexion = new JButton("Se connecter à un serveur");
	...
	public void modeDeconnecte(){ // Gere uniquement l'AFFICHAGE graphique lors de la deconnexion
		...
		bConnexion.setText("Se connecter à un serveur");
	}
	...
	public void actionPerformed(ActionEvent a) {
		...
		if(a.getSource() == fenetre.bConnexion){
			...
			if(!MainClient.client.isConnecte()){
				//*
				BdDialogueConnexion jop = new BdDialogueConnexion(null, "Connexion à un serveur", true);
				...
				int option = JOptionPane.showConfirmDialog(null, "Voulez-vous vous déconnecter du serveur ?", "Deconnexion du serveur", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
	...
	public void keyReleased(KeyEvent event) {
		if(event.getKeyCode() == 10 && zoneTexte.getText().length() > 0){ //Si on appuie sur Entree et qu'on a ecrit qqchose
			if(zoneTexte.getText().substring(0, zoneTexte.getText().length()-1).contains("\n")) 
				zoneTexte.setText(zoneTexte.getText().replace("\n".charAt(0), ' '));//On enlève les '\n' qui posent problème à l'envoi
...
class EcouterFermetureFenetre extends WindowAdapter{
	public void windowClosing(WindowEvent w) {
	    ...
	    MainClient.fenetre.dispose();     // libération des ressources utilisées par la frame
	    System.exit(0);
	  }
}
