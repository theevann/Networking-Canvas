...
public class OngletHistorique extends JPanel implements ActionListener, MouseListener{
	...
	private static String  title[] = {"N°","Nom", "Type", "Couleur", "Visible"};
	...
	public void synchroniser(int [] num) {
		//1. Regarder si l'un des elements selectionnes est supprimé et le suppimer.
		//2. Enlever une unité dès qu'ils sont au dessus d'un element supprimé
		...
		// On enlève de la liste des éléments selectionnés ceux qui ont été supprimés
		...
		//On calcule la nouvelle position des éléments à selectionner
		...
		for(int j : listeElmtSelect){
			...
			for(int i = 0 ; i < num.length ; i++){
				if(num[i] < j){
					...
					//Pour cela, on enregistre combien de nombre a supprimer sont inferieur a j en incrementant sup (le decalage à operer) si c'est le cas.
					sup++;
				}
			}
			if(sup > 0){
				//On modifie la valeur de j, mais comme ce n'est pas possible "facilement", on l'enlève et on remet la valeur modifie apres
